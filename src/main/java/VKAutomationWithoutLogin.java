...
public class VKAutomationWithoutLogin {
    public static String delcity(String cityname){
        ...
        cities.add("\n");
        cities.add("@https://vk.com/wall-38313115_108325 (Подмосковье)\n");
        ...
        cities.add("@https://vk.com/app5898182_-35055247#page=66e1a747c59520404971c1f2 (Ростов)\n");
        ...
        cities.add("\n");
        cities.add("@https://vk.com/app5898182_-96824879#page=671f79d7ddcfb12190380092 (Абакан)\n");
        cities.add("@https://vk.com/app5898182_-138456139#page=66d84b6a8fffe75b0e7e3612 (Астрахань)\n");
        cities.add("@https://vk.com/app5898182_-148251442#page=66d86b043cd94c613d66e1a2 (Барнаул)\n");
        ...
        cities.add("@https://vk.com/app5898182_-123100662#page=66d87315ee24bf22c81741f2 (Дзержинск)\n");
        ...
        cities.add("\n");
        cities.add("@https://vk.com/app5898182_-161640865#page=671f7edf7a7c1900212db453 (Таганрог)\n");
        ...
        cities.add("@https://vk.com/app5898182_-184425431#page=671f7f42883f7f0acc231bb2 (Череповец)\n");
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            List<String> urls = List.of(
                    ...
                    "https://vk.com/typical_cherepovets",
            ...
            //table[contains(@class,'cal_table')]/tbody/tr/td[contains(@onclick,'return cals.getDay(undefined, 8, 11, 2024);')]
            // Перебор всех URL
            int index = 0;
            for (String url : urls) {
                ...
                driver.get(url);
                try {
                    // Находим поле для публикации и вводим текст поста
                    Thread.sleep(550);
                    driver.findElement(By.xpath("//*[@id='post_field']")).click();
                    if (driver.findElements(By.xpath("//span[@class='post_action_image_btn _close']")).size() > 0) {
                        WebElement nowButton = driver.findElement(By.xpath("//span[@class='post_action_image_btn _close']"));
                        nowButton.click();
                        Thread.sleep(550);
                        driver.get(url);
                        Thread.sleep(550);
                    ...
                    // Шаг 1: Установка времени публикации
                    WebElement nowButton = driver.findElement(By.xpath("//span[@class='post_action_btn_text' and @aria-label='Сейчас']"));

                    //ScrollPage(0, element.getLocation().getY());
                    nowButton.click();
                    Thread.sleep(2000);  // Ждём открытия выпадающих списков

                    //table[contains(@class,'cal_table')]/tbody/tr/td[contains(@onclick,'return cals.getDay(undefined, 8, 11, 2024);')]
                    //table[contains(@class,'cal_table')]/tbody/tr/td[contains(@onclick,'return cals.getDay(undefined, "+PostDay+", "+PostMonth+", "+PostYear+");')]
                    WebElement dateInput = driver.findElement(By.xpath("//table[contains(@class,'cal_table')]/tbody/tr/td[contains(@onclick,'return cals.getDay(undefined, 30, 12, 2024);')]"));
                    dateInput.click();

                    // Выбор часов
                    WebElement hoursInput = driver.findElement(By.xpath("//input[@class='selector_input selected' and @aria-labelledby='selector_placeholder_content' and @aria-owns='list_options_container_1']"));
                    hoursInput.click();
                    WebElement hourOption = driver.findElement(By.id("option_list_options_container_1_1")); // Устанавливаем час "0"
                    hourOption.click();

                    // Выбор минут
                    WebElement minutesInput = driver.findElement(By.xpath("//input[@class='selector_input selected' and @aria-labelledby='selector_placeholder_content' and @aria-owns='list_options_container_2']"));
                    minutesInput.click();
                    WebElement minuteOption = driver.findElement(By.id("option_list_options_container_2_15")); // Устанавливаем минуту "01"
                    minuteOption.click();

                    // Шаг 2: Загрузка изображения
                    WebElement fileInput = driver.findElement(By.xpath("//input[@type='file']"));
                    fileInput.sendKeys("C:\\image.jpg");

                    // Ждём, пока изображение загрузится (визуально или используем ожидание)
                    Thread.sleep(5500);
                    wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("post_field")));
                    driver.findElement(By.id("post_field")).sendKeys(delcity(getcityname(index)));
                    Thread.sleep(2500);

                    // Шаг 3: Нажимаем кнопку "В очередь" через JavaScript
                    WebElement queueButton = driver.findElement(By.id("send_post"));
                    JavascriptExecutor js = (JavascriptExecutor) driver;
                    js.executeScript("arguments[0].click();", queueButton);

                    System.out.println("Фото загружено, время выставлено, и кнопка 'В очередь' нажата на странице: " + url);

                    index++;
                } catch (Exception e) {
                    System.out.println("Не удалось найти или взаимодействовать с элементами на странице: " + url);
                    e.printStackTrace();
                } finally {
                    // Добавляем 2 секунды задержки перед закрытием вкладки
                    Thread.sleep(4500);

                    // Закрытие вкладки после завершения всех действий с текущей страницей
                    driver.close();
